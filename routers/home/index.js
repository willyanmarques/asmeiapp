...
function handlerLoadPosts() {
    ...
    getCategoryIdBySlug('home').done(function (category) {
        ...
        getPostsByCategotyId(category[0].id).done(function (posts) {
            ...
            var cardLogout = $("<div class='card-logout'></div>")
            .append(
                $("<div class='img-container'></div>")
                    .append(`<img src="icons/logout.png"></img>`)
            )
            .append(
                 $("<div class='title-container'></div>")
                 .append("<span>Sair do APP</span>")
             )

            $("article").append(cardLogout);
...
function handlerClickCardLogout() {

    $(document).on("click", ".card-logout", function () {
        logout();
    });

}

handlerClickCardLogout();
